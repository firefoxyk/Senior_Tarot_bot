...
- `/stats` исключает действия админов из счетчиков активности, карт и раскладов, чтобы тесты админа не попадали в продуктовую статистику.
...
В группах Telegram при включённом privacy mode бот обычно видит команды `/...`, ответы на свои сообщения и inline-кнопки под сообщениями, но не обязан получать обычный текст от reply-клавиатуры. Поэтому в группах бот показывает только inline-меню под сообщением и подсказку использовать кнопки под сообщением или команды через `/`.

В приватном чате остаются оба варианта меню: reply-клавиатура снизу и inline-кнопки под сообщением.
